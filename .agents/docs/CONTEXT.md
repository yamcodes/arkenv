...
- **Next.js**: Integrated via `@arkenv/nextjs`. **Flat layout** is the only first-class pattern (single `env.ts`). `withArkEnv` generates `@/.arkenv` with `runtimeEnv` filled, `NEXT_PUBLIC_` prefixing, and value isolation via conditional exports + proxy. Name/type isolation is a documented two-module recipe ([ADR 0020](../../docs/adr/0020-strict-layout-complexity-budget.md) / [#1690](https://github.com/yamcodes/arkenv/issues/1690)): client via codegen, server via `@arkenv/core` plus optional `import "server-only"`. Dedicated `--strict` / `/client` `/server` were removed.
