# ADR 0001: Bun plugin configuration patterns
...
To support build-time environment variable validation in Bun environments.

## Decision

The Bun plugin supports two primary configuration patterns:
...
- Allows both programmatic builds and global runtime server configurations.
- Reconciles configuration differences between build/run times.
