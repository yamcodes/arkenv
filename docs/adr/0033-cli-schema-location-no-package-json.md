...
2. **Do not read** `package.json` `"arkenv"` during schema discovery.
   Resolution is `--schema` / `-s` → convention candidates only.
3. **Ignore leftovers**: if an older project still has the field, the CLI
   does not fail on it and does not use it.
4. **Keep the boundary clear**: scripts and `--schema` (or a convention
   path) locate the schema for the CLI; TypeScript `arkenv()` options own
   runtime behavior. `package.json` is not a CLI config surface for schema
   location.
