...
Each of the three things removed only provides value in a configuration that ArkEnv structurally cannot satisfy:

**Declaration maps** require the published tarball to also contain the raw `.ts` source files that the maps point to. ArkEnv publishes compiled output only — the source files are in the repository but are never included in the npm artifact. A `.d.ts.map` file with no corresponding `.ts` source to resolve to is inert: IDEs silently ignore it. There is no user workflow that benefits from declaration maps without accompanying sources; removing them is loss-free.

**Runtime sourcemaps** exist to re-map line numbers in minified, mangled bundles back to the original source. ArkEnv does not minify: identifiers are readable, line numbers are meaningful, and the bundle is human-inspectable. The other use case — server-side crash symbolication — applies to applications, not libraries: it is the application's own build pipeline that produces the sourcemaps a crash reporter reads. A library's sourcemaps would only matter here if the application bundled the library's raw source files (e.g. via a `src/` export condition), which ArkEnv does not offer. Removing them produces no change in any developer's debugging workflow.

**ArkType AST inlining in declarations** was never intentional — it was a latent bug triggered by referencing private generic parameters without explicit annotations. The fix (pinning to `Type<U, any>` and `ArkErrors`) does not change what `@arkenv/core` exports or how it behaves at runtime; it only changes how `tsc` encodes those exports in the `.d.mts` file. Every value and type that was part of the public API before is still part of the public API after. The type surface for end-users is identical; the file that encodes it is simply much smaller.
...
- Declaration files now reference ArkType's stable public `./internal/` subpath exports rather than expanding AST internals, significantly reducing IDE language-server parse time for consumers.
