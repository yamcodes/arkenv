...
- **Next.js**: Integrated via `@arkenv/nextjs`. Provides a standalone `createEnv` helper that validates environment variables at runtime and during the build phase. Enforces strict client-side prefixing (`NEXT_PUBLIC_`) and prevents server secrets from leaking to client components by throwing runtime errors in the browser.
