...
- **Next.js**: Integrated via `@arkenv/nextjs`. Provides a standalone `createEnv` helper that validates environment variables at runtime and during the build phase. In Next.js, client-side environment variables must be statically destructured in a `runtimeEnv` block to allow static inlining by the Next.js compiler. To automate this, `@arkenv/nextjs/config` exposes a `withArkEnv` wrapper for `next.config.js` that performs static analysis on `env.ts` to locate `client` and `shared` keys, then automatically generates a tailored `createEnv` factory in `generated/env.gen.ts` that pre-fills `runtimeEnv`. This allows developers to define schemas in `env.ts` and import from `./generated/env.gen` without manually writing any `runtimeEnv` boilerplate. It enforces strict client-side prefixing (`NEXT_PUBLIC_`) and prevents server secrets from leaking to client components by throwing runtime errors in both browser and server-side pre-rendering (SSR) environments using package conditional exports (`react-server` vs. `default`).
