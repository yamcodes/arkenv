...
function saveMetrics(metrics) {
    if (isDryRun) {
        console.log('[Dry Run] Would save metrics:', JSON.stringify(metrics, null, 2));
        return;
    }
...
async function run() {
    ...
    if (type === 'preview') {
        ...
        // Check cooldown
            ...
            const latest = new Date(metrics.preview[metrics.preview.length - 1]).getTime();
        ...
        if (shouldDeploy) {
            ...
            console.log(`Preview deployment skipped. Reason: ${reason}. Remaining cooldown: ${remainingTime}`);
        }
        ...
        if (count >= LIMIT_24H_PROD) {
            console.error(`Alert: Production deployment budget reached! (${count}/${LIMIT_24H_PROD} in 24h)`);
            process.exit(1);
        }
        console.log(`Production deployment logged. (${count}/${LIMIT_24H_PROD} in 24h)`);
    }
}
