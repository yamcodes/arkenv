...
async function run() {
  const results: Record<string, Array<{ id: string; name: string; bytes: number; kb: string; tier: string }>> = {
    full: [],
    adapter: [],
  };

  for (const [mode, tests] of Object.entries(cases) as [("full" | "adapter"), TestCase[]][]) {
