...
Modern IDEs may still infer types from ArkEnv’s `.d.ts` files, but this is not something we explicitly guarantee or test for. If it works well for you, great. For a more predictable experience, we recommend [using TypeScript](https://arkenv.js.org/docs/arkenv/quickstart#configure-typescript).
