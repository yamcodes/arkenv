...
> [!NOTE]
> This is a version of the [basic example](https://github.com/arkenv/arkenv/tree/main/examples/basic) that works on Stackblitz.
> We changed the way we run the script from `tsx` to `ts-node` to make it work on Stackblitz on Firefox, where WebAssembly Core is still in beta.
