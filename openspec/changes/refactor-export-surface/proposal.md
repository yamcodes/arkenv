...
- **ADDED** `arkenv/standard` entry (`src/standard.ts`): a clean, ArkType-free `createEnv` for Standard Schema users. The standard-mode runtime guards (string-is-DSL check, `~standard` check) live here; there is no ArkType import anywhere in its module graph.
...
- **ADDED** `ARCHITECTURE.md` at repo root documenting the three-tier export surface, ownership rules, and reasoning.

## Impact

- Affected specs: new `export-surface` capability; modified `validator-mode`
- Affected code: `packages/arkenv/src/index.ts`, `packages/arkenv/package.json`, `packages/arkenv/tsdown.config.ts`, new `src/standard.ts`, new `src/core.ts`
