...
### Requirement: Vite Plugin Environment Variable Exposure
...
- **THEN** the plugin validates these variables at build-time
- **AND** validated variables are exposed to client code via `import.meta.env.*`
- **AND** only variables matching the configured prefix are exposed
...
- **AND** they pass a schema to the Vite plugin
- **THEN** the plugin uses the configured prefix to filter variables
