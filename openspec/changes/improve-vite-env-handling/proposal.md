...
Currently, the `@arkenv/cli` has a frustrating UX when handling the `vite-env.d.ts` or `bun-env.d.ts` files. It asks a hypothetical question ("Establish vite-env.d.ts?"), and if the user says yes but the file already exists, it hits them with a destructive overwrite warning. This forces developers to manually check their filesystem and can lead to accidental deletion of default Vite or Bun configurations.

## What Changes

- **Smart Detection**: The CLI will check for the existence of `vite-env.d.ts` or `bun-env.d.ts` before prompting the user.
...
- **Improved UX**: Users are no longer asked to "Establish" a file that already exists, and the default action becomes non-destructive for both Vite and Bun users.
