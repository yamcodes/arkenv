...
- Update the `yamcodes/arkenv` skill instructions to include an explicit "Agent Setup" section instructing the LLM to run `pnpm dlx @arkenv/cli@latest init --yes` rather than writing configuration files manually.
...
- `cli-agent-mode`: Non-interactive `--yes` / `--quiet` / `--json` flags on `@arkenv/cli init` that make the CLI fully machine-readable and safe to invoke from an AI agent context.
