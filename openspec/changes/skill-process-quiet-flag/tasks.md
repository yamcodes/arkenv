...
- [ ] 1.2 Modify the invocation logic to automatically append the `--yes` flag to skill process arguments.

## 2. Quiet Mode Implementation with Error Capture

- [ ] 2.1 Update the spawn/exec options to conditionally use `stdio: 'pipe'` instead of streaming to the terminal when quiet mode is enabled.
...
- [ ] 3.1 Write tests to verify that `--yes` is passed correctly to skill processes.
