...
- Homepage `<title>` is punchy: "ArkEnv - Typesafe environment variables for TypeScript" (ASCII hyphen, not an en-dash). Library names (ArkType, Zod, Valibot, Standard Schema) go in `<meta name="description">`, not the title. Middle dots (`·`) are for on-page text only.
