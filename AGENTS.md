...
- Homepage TypeScript snippets (hero, 01 compare, 03 schema/bundle, 04 Modular) use the same Twoslash highlighter as the docs (`highlightTwoslash` + `HeroTwoslashHtml`). The 02 IDE autocomplete mock and 05 CLI/transcript are not Twoslash. Pitch sections have no numbered kickers. Headings: “One schema replaces your manual checks.” · “Fail fast at boot. Autocomplete everywhere.” · “Prevent accidental client leaks.” · “Keep your existing validator.” Then the playground (`#demo`), then Agent-ready, FAQ, outro. On viewports ≥64rem, each pitch shares the hero split tracks (copy | visual) so the code windows share one right edge. Nested IDE/terminal splits stack in the visual column. Homepage tabs (hero, Modular, compare, secure) share the sliding-ink tablist. Code windows share `home-aurora__code-window`. Inline links match the footer: ink-2, hover to ink, no underline. Error terminals use `home-aurora__fail`, not red/zinc utilities. Hero example tabs are ArkType / Zod only; Modular (04) has ArkType / Zod / Valibot with `NODE_ENV`, `DATABASE_URL`, and `LOG_LEVEL` (enums + URL, no numeric coercion). "Typesafe environment variables" / "with [cycling name]" on one line (ArkType, Zod, Valibot, Standard Schema) — never wrap between “with” and the name. That cycle is independent of the example tabs (ArkType / Zod only) and pauses while the pointer is over the headline or the example. Dwell is 3s; the name slide is 375ms `cubic-bezier(0.16, 1, 0.3, 1)`. Reduced motion locks to ArkType. The example window is vanilla `./env.ts` only — no Vanilla/Vite/Next.js switcher. The hero trust bar pairs Colin’s quote with an inline “Works with” + logo marquee; the marquee is the full ecosystem list (runtimes, editors, frameworks, and all validators including Valibot) and is not 1:1 with the tabs. Both hero tabs use the same keys (`DATABASE_URL`, `PORT`, `CI`); ArkType PORT is `"0 <= number.integer <= 65535 = 3000"` (not `number.port`) and CI is `"boolean = false"` so switching tabs is a git-diff of the same rules. Hero install: command pill is always `$ npx arkenv@latest init`; a sub-row under it is “Copy prompt / View repo”. Outro is a centered finish-line card: “Try ArkEnv now.” + subline, the same command pill, then “Copy prompt / Read docs”. No humans/agents tabs — the example window is the only tab bar.
