...
describe("arkenv (RSC / Server Entrypoint)", () => {
	...
	it("should allow accessing server-only, client, and exposed variables on the server", () => {
		const env = serverArkenv(
			{
				DATABASE_URL: "string",
...
describe("arkenv (Client / SSR Entrypoint)", () => {
	...
	describe("Flat Mode", () => {
		...
		it("should ignore removed expose and shared option aliases", () => {
			const env = clientArkenv(
				{
					DATABASE_URL: "string",
					NEXT_PUBLIC_API_URL: "string",
			...
			expect(env.NEXT_PUBLIC_API_URL).toBe("https://api.example.com");
			expectBoundaryAccessError(() => (env as any).CUSTOM_VAR, "CUSTOM_VAR");
		});
	});
});
