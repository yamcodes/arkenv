...
describe("build-time environment validation", () => {
	...
	describe("strict layout", () => {
		...
		it(
			"should pass when strict layout variables are all valid",
			() => {
				fs.writeFileSync(
					sharedPath,
					`
		...
		it(
			"should throw error when a server variable is missing in strict layout",
			() => {
				fs.writeFileSync(
					sharedPath,
					`
		...
		it(
			"should still honor explicit extends in strict layout validation",
			() => {
				fs.writeFileSync(
					sharedPath,
					`
