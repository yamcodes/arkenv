...
describe("committed bin shim", () => {
	it("exists so pnpm can link the CLI before dist is built", async () => {
		...
		) as { bin: { "arkenv-agent-plugin": string }; files: string[] };
		expect(pkg.bin["arkenv-agent-plugin"]).toBe("./bin.mjs");
	...
	it("prints a build hint when dist/bin.js is missing", async () => {
		const dir = await mkdtemp(path.join(tmpdir(), "arkenv-agent-plugin-bin-"));
		try {
