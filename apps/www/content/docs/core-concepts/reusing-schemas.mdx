...
`extends` merges shared and client definitions into the server env object. Use `@arkenv/nextjs/standard/server` (and matching client entries) when the schema is Standard Schema. See [client vs. server](/docs/core-concepts/client-vs-server#strict-layout).
