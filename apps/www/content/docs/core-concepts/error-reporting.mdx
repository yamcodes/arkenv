...
<Callout type="warn">
  Framework plugins (Next.js, Nuxt, Vite, Bun, Rsbuild) do not ship a
  `/safe` subpath, so a failed validation cannot soft-fail into the
  client bundle — invalid env still fails the build or server start on
  purpose. Call `arkenv` from `@arkenv/core/safe` or
