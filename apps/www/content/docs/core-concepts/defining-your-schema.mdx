...
application code runs, whether you use ArkType, Zod, or Valibot.

## Fail fast

Calling `arkenv()` parses the configured source immediately. Missing
required keys or values that fail the schema throw
[`ArkEnvError`](/docs/core-concepts/error-reporting) at
the call site.

| Moment               | What runs                                                                                  |
| -------------------- | ------------------------------------------------------------------------------------------ |
| Module load          | `arkenv()` validates the current `env` source (default `process.env`)                      |
| Before the entry     | Optional: `arkenv check`, or Nub `preload` — see [Use with Nub](/docs/guides/use-with-nub) |
...
[hosting presets](/docs/core-concepts/hosting-presets) can
...
[flat layout](/docs/core-concepts/client-vs-server#flat-layout-recommended)
frameworks:
...
<Tabs items={["ArkType (@arkenv/core)", "Zod (@arkenv/standard)", "Valibot (@arkenv/standard)"]}>
  <Tab value="ArkType (@arkenv/core)">
    ...
    Empty collections are defaults in the same string. ArkType 2.2.5
    accepts `= []` and `= {}`, and each missing value gets a fresh array
    or object:
    ...
    On ArkType older than 2.2.5, that string fails to parse. Use
    `type("string[]").default(() => [])` there. Non-empty lists still
    need that function form, because `= ["localhost"]` is not valid
    string syntax.
  </Tab>
  ...
  <Tab value="Valibot (@arkenv/standard)">
    ...
    export const env = arkenv({
    	...
    	APP_NAME: v.optional(v.string(), "My App"),
    	OPTIONAL_NOTE: v.optional(v.string()),
    	PORT: v.optional(
    		v.pipe(v.number(), v.integer(), v.minValue(0), v.maxValue(65535)),
    		3000,
    	),
    ...
    A bare schema requires the key. `v.optional(schema, fallback)` supplies
    a default when the key is missing. `v.optional(schema)` accepts a
    missing key. An empty string (`NOTE=`) still counts as present unless
    you set [`emptyAsUndefined`](/docs/reference/options).

    Empty collections are defaults on the same field. Each missing value
    gets the fallback you pass:
    ...
    export const env = arkenv({
    	FEATURE_FLAGS: v.optional(v.array(v.string()), []),
    	METADATA: v.optional(v.record(v.string(), v.unknown()), {}),
    });
    ```

    Import from [`@arkenv/standard/valibot`](/docs/validators/valibot) so
    coercion is wired. The root `@arkenv/standard` import does not bind
    Valibot's JSON Schema converter.
  </Tab>
</Tabs>
...
<Tabs items={["ArkType type()", "Zod", "Valibot"]}>
  <Tab value="ArkType type()">
    ...
    export const env = arkenv({
    	PORT: "number.port = 3000",
    	ALLOWED_HOSTS: type("string[]").default(() => ["localhost"]),
...
See
[Transforms](/docs/core-concepts/coercion-and-parsing#transforms)
for ArkType morphs, Zod `.transform`, and Valibot `v.pipe`, and
[reusing schemas](/docs/core-concepts/reusing-schemas)
...
See [Validators](/docs/validators) for
packaging and peer-dependency differences.

## Next steps

<Cards>
  <Card href="/docs/core-concepts/client-vs-server" title="Client vs. server" description="Keep public keys and private secrets in separate buckets." />
  ...
  <Card href="/docs/validators" title="Validators" description="When to use @arkenv/core vs @arkenv/standard." />
</Cards>
