...
ArkEnv ships two engines. Both call `arkenv()`, share fail-fast errors, the coercion pipeline, and framework plugins. Most [options](/docs/reference/options) apply to both; `toJsonSchema` is **standard-mode only**. They differ in how you write the schema and which peer dependencies you install.
...
Neither path is a fallback. Pick the engine that matches the schema library you want to write in. Cookbooks: [Validators](/docs/guides/validators).
...
| Zod Mini                                       | Via probes / helpers (see library docs)                              |
