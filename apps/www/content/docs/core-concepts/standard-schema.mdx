...
| Valibot (v1.2+)                          | Yes, via `toStandardJsonSchema` from `@valibot/to-json-schema` |
| Zod Mini                                 | Yes, via `z.toJSONSchema(...)`                             |
| VineJS (v4.3+)                           | Yes                                                        |
| Other Standard JSON Schema v1 validators | Yes                                                        |

Standard Schema alone is not enough for ArkEnv coercion. Pass values that satisfy Standard JSON Schema (including wrappers like Valibot’s `toStandardJsonSchema`), or use the validator’s own coerce helpers / set `coerce: false` and [transform](/docs/core-concepts/transforms) strings yourself. See [Coercion](/docs/core-concepts/coercion).
