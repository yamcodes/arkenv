...
Same two-module shape. There is no compile-time import blocker: **never
import the server module from client or Vue code.** Discipline is the
fence.
...
Register `@arkenv/nuxt/module` and point the module schema path at the
client (or flat) entry your app actually loads on both sides.
