...
(`env.ts`, `src/env.ts`, and related extensions).
...
paths only (`env.ts`, `src/env.ts`, and related extensions). Split
recipes such as `env/server.ts` are not auto-discovered — pass
`--schema`. Put custom locations in your scripts with `--schema` (for
