...
Explicit path to the schema module. Overrides convention discovery
(`env.ts`, `src/env.ts`). Load a JavaScript or ESM schema (for example
`--schema ./env.js`) by passing its path explicitly. Split-layout leftovers
such as `env/server.ts` are not auto-discovered. For a two-module recipe,
pass `--schema` at the client (or other loadable) module — not a server
file that imports `server-only`, which the CLI cannot evaluate in plain
Node.
...
When you omit `--schema`, `check` looks for a schema at flat convention
paths only. Put custom locations in your scripts with `--schema` (for
example `"check": "arkenv check --schema config/env.ts"`). The CLI
does not read a `package.json` `"arkenv"` field for discovery.
