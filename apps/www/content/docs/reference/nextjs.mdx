...
| `./standard`        | Standard Schema unified entry (`react-server` for RSC, default for client)  |
...
`.` and `./standard` publish the same export conditions. Next.js resolves
`react-server` in Server Components and Route Handlers, so those imports
see every schema key. Client Components and SSR resolve `default`, which
exposes public keys only. Generated `env.gen.ts` still imports
`@arkenv/nextjs` or `@arkenv/nextjs/standard`. The bundler selects the
build from the importing context.
