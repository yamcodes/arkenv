...
| `SafeArkEnvResult`  | type                       | Result shape returned by `@arkenv/standard/safe` |
| `StandardEnvConfig` | type                       | Extends core options with `toJsonSchema`         |
...
| `@arkenv/standard/issues`   | `formatIssues`, `getSchemaKeys`, and `EnvIssue` / `EnvIssueCode` / `EnvIssueMeta`              |
