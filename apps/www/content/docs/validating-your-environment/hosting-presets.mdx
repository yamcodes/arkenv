...
Hosting providers inject their own system environment variables at build
and runtime. Presets teach the ArkEnv CLI which keys to add, already typed
and optional, so you do not look them up by hand.

Presets work with ArkType, Zod, and Valibot, and with both [flat and strict](/docs/validating-your-environment/client-vs-server) layouts.

Not on this list? Choose **None** in the interactive CLI (or skip adding a
host later) and declare the provider's variables yourself in the schema
the same way you declare `DATABASE_URL`. Presets are a convenience for
common hosts, not a requirement.

## Select a preset during init

### Interactive prompt

The interactive CLI asks for a hosting provider when you run it without
flags:
...
After `npx arkenv@latest init -H vercel` on a Next.js app, the generated
schema includes the preset fields alongside your own:
...
Exact output varies by validator and framework. Re-run
`npx arkenv@latest add host` when you move providers instead of copying
keys by hand.

## Next steps

<Cards>
  <Card href="/docs/reference" title="API reference" description="Comprehensive API, CLI, and package export reference." />

  <Card href="/docs/core-concepts" title="Core concepts" description="Understand coercion, transforms, and Standard Schema validation." />
</Cards>
