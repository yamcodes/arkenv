...
title: Introduction
icon: Album
description: The ArkEnv plugin for Vite lets you validate environment variables at build-time with ArkEnv.
---
...
Define your schema as an exported constant in `vite.config.ts`. This allows the schema to be imported for type safety in other files.
...
You can now use `import.meta.env` with full type safety and autocompletion throughout your application.
...
type Formatted = ImportMetaEnvAugmented<typeof Env>;
//    ^?
```
