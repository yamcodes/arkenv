...
description: Learn how to use ArkEnv in a Bun fullstack application.
---

ArkEnv supports most Bun apps out of the box, no further configuration required.

However, some Bun apps use [Bun's fullstack dev server](https://bun.com/docs/bundler/fullstack) (`Bun.serve()`) or [Bun's bundler](https://bun.com/docs/bundler) (`Bun.build()`). The Bun integration was built to support these use cases.
...
Valibot, then import the plugin from `@arkenv/bun-plugin/standard`:
...
<Tabs items={["ArkType", "Zod", "Valibot"]}>
  ...
  <Tab value="Zod">
    ...
    export const env = arkenv({
    	...
    	BUN_PUBLIC_DEBUG: z.boolean().default(false),
    });
    ```
  </Tab>
  ...
  <Tab value="Valibot">
    ...
    export const env = arkenv({
    	...
    	BUN_PUBLIC_API_URL: v.string(),
    	BUN_PUBLIC_DEBUG: v.optional(v.boolean(), false),
    });
    ```
  </Tab>
</Tabs>
...
plugin and call the runtime directly:
...
- **Server execution:** Server entrypoints and backend APIs execute real `@arkenv/core` or `@arkenv/standard` validation at startup, validating database URLs and backend secrets before serving traffic.
...
<Cards>
  ...
  <Card href="/docs/core-concepts/hosting-presets" title="Hosting presets" description="Inject hosting provider system variables into your Bun schema." />

  <Card href="/docs/core-concepts/client-vs-server" title="Client vs. server" description="Understand client versus server security boundaries." />
</Cards>
