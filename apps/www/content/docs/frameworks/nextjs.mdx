...
`@arkenv/nextjs/standard` binds the root `@arkenv/standard` engine and
publishes the same `react-server` condition as `.`. Server Components
and Route Handlers resolve the full schema. Client code resolves public
keys only. Zod 4.2+ coerces numbers and booleans from that entry.
Valibot does not: there is no `@arkenv/nextjs/standard/valibot` binding,
so installing `@valibot/to-json-schema` does not turn `v.number()` or
