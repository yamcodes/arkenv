...
Zod and Valibot both import `withArkEnv` from the `/standard/config`
subpath. Valibot schemas also need the app to resolve
`@arkenv/standard/valibot` (install `valibot` and
`@valibot/to-json-schema`). The schema itself stays on `@/.arkenv`, as
in the tabs above.
