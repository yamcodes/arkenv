...
the plugin matching your project setup.
...
function Home() {
  ...
  return (
    <div>
      <h1>API: {env.VITE_API_URL ?? env.PUBLIC_API_URL}</h1>
...
```tsx
env.VITE_API_URL; // string, inlined into the client bundle (Vite)
env.PUBLIC_API_URL; // string, inlined into the client bundle (Rsbuild)
