...
plugin, an `env.ts` schema, and ArkType by default.
...
<Callout type="info" title="Older CLI builds">
  The bare `arkenv` id needs a `@tanstack/cli` release that includes the
  catalog add-on. On an older CLI, pass
  `https://arkenv.js.org/tanstack/info.json` instead of `arkenv`.
</Callout>
