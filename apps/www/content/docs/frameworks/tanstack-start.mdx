...
const readDatabaseUrl = createServerFn({ method: "GET" }).handler(() => {
  return env.DATABASE_URL; // server-only: validated at boot
});
...
The [`with-tanstack-start`](https://github.com/yamcodes/arkenv/tree/main/examples/with-tanstack-start)
example runs the setup above end to end: a server-only `DATABASE_URL` read
through `createServerFn`, public `VITE_` keys rendered in a client component,
