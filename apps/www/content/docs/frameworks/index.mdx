...
SSR apps.
Application code imports `@arkenv/core` (ArkType), `@arkenv/standard`
(Zod), or `@arkenv/standard/valibot` (Valibot); the
...
For NestJS applications, `@nestjs/cli` 11+ supports
`nest start --env-file .env` — follow the
[Using ArkEnv with NestJS](/docs/guides/use-with-nestjs) guide.
