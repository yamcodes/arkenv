...
Node's `node --env-file=.env` (and `nest start --env-file=.env` for NestJS) also works. You name each file yourself,
and it does not run TypeScript. For NestJS applications, follow the [Use with NestJS](/docs/guides/use-with-nestjs) guide.
