...
<Accordions>
  <Accordion title="Manual Installation">
    ...
    <Accordions defaultValue="Install ArkEnv + ArkType (recommended) + Plugins (optional)">
      ...
      <Accordion title="Install @arkenv/standard (ArkType-free)">
        ```package-install
        @arkenv/standard
        ```

        Import from `@arkenv/standard` to use ArkEnv without ArkType.
      </Accordion>
    </Accordions>
...
Notice that ArkEnv extends ArkType with env-oriented [keywords](/docs/arkenv/keywords) such as `string.host` and `number.port`.
