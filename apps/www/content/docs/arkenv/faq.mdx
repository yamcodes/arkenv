...
## Do I need to install `arktype`?

**Only if you use `@arkenv/core`.** Zod, Valibot, and other Standard Schema users should install [`@arkenv/standard`](/docs/arkenv/standard) instead — no `arktype` peer dependency.

Framework integrations follow the same split via `/standard` subpaths:

- Next.js → [`@arkenv/nextjs/standard`](/docs/nextjs/using-other-validators)
- Nuxt → [`@arkenv/nuxt/standard`](/docs/nuxt/using-other-validators)
- Vite → `@arkenv/vite-plugin/standard`
- Bun → `@arkenv/bun-plugin/standard`

See the [with-zod example](https://github.com/yamcodes/arkenv/tree/main/examples/with-zod) for a complete ArkType-free setup.
...
Still, we support any modern validation library through [Standard Schema](/docs/arkenv/integrations/standard-schema), so even if you're not as excited about ArkType as we are (that's ok!), you're more than welcome in the ArkEnv community. See [Do I need to install `arktype`?](#do-i-need-to-install-arktype) if you want an ArkType-free setup.
...
The `arkenv` package **automates setting up ArkEnv in your project**. It can scan your workspace to detect your framework and **generate a fully typed environment schema**, or scaffold a new, ArkEnv-compliant project from scratch.
