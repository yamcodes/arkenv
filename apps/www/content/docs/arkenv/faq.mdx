...
## Why do you guys like ArkType so much?

That's a great question! We're big ArkType nerds because it's a revolutionary validation library, that pretty much "brings TypeScript to runtime".
...
This string-based syntax makes defining schemas incredibly intuitive and readable. Instead of chaining endless method calls like `.string().url()` or `.number().min(1024)`, you simply write strings that describe your types.

How clean is that for validation API? Honestly, that's why ArkEnv exists, as a way to honor this beautiful feat of engineering.

Still, we support any modern validation library through Standard Schema, so even if you're not as excited as us (that's ok!), you're welcome in the ArkEnv community.
