...
ArkEnv uses a unified pre-coercion execution model for both the default `arkenv` (ArkType) and `arkenv/standard` (Standard Schema) entry points:
...
Yes! Coercion is available in both `arkenv` and `arkenv/standard`. They share the exact same pre-coercion pipeline and coercion logic.
