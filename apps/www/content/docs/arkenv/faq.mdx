...
We built ArkEnv to **eliminate this exact class of failures**. With a single initialization command (`npx arkenv@latest init`), you establish a **fail-safe mechanism** that guarantees your runtime configuration exactly mirrors your technical specification **before a single line of application code executes**.
...
## How does coercion work under the hood?

ArkEnv uses a unified pre-coercion execution model for both the default `@arkenv/core` (ArkType) and `@arkenv/standard` (Standard Schema) entry points:

1. **JSON Schema Introspection**: At boot time, ArkEnv extracts a JSON Schema from the validator.
2. **Path Identification**: It traverses the JSON Schema (using `findCoercionPaths`) to identify which paths (like numbers, booleans, arrays, or objects) require type conversion.
3. **Targeted Pre-Coercion**: ArkEnv creates a shallow copy of the input environment object and applies coercion (using `applyCoercion`) to identified paths (e.g., converting `"3000"` to `3000` or `"true"` to `true`).
4. **Validation**: The coerced copy is validated against the original, unmodified schema.

## Is coercion available in `@arkenv/standard`?

Yes! Coercion is available in both `@arkenv/core` and `@arkenv/standard`. They share the exact same pre-coercion pipeline and coercion logic.

The main difference lies in how they extract JSON Schema:

- **ArkType**: Introspects the schema with an unjsonifiable fallback (preserving custom scopes and exotic types like predicates or custom morphs).
- **Standard Schema**: Relies entirely on the validator's JSON Schema output via the standard metadata protocol (`validator["~standard"].jsonSchema.input()`).
...
The `@arkenv/core` package **automates setting up ArkEnv in your project**. It can scan your workspace to detect your framework and **generate a fully typed environment schema**, or scaffold a new, ArkEnv-compliant project from scratch.
