...
- In Next.js, T3 Env requires you to manually maintain `runtimeEnv` block (e.g. `NEXT_PUBLIC_VAR: process.env.NEXT_PUBLIC_VAR`) so Next.js can inline variables. This violates the principle of locality - any time you change a variable, you have to remember to update both the schema and the `runtimeEnv` block. ArkEnv's compiler automatically scans your schema and generates these mappings for you.
- T3 Env enforces a strict client/server split. You are always forced to group keys into `server` and `client` blocks, which is redundant for simple Node.js servers and other non-Next.js usecases. ArkEnv uses a flat, simple config by default and securely splits between client/server variables for fullstack frameworks like Next.js. 
- ArkEnv has even more features: native ArkType synergy, automatic coercion, first-class Vite integration, and a CLI out of the box.
