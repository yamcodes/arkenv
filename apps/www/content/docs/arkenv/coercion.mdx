...
:::note
In `arkenv/standard` (the ArkType-free entry point), coercion is opt-in and requires setting `coerce: true` in your configuration. Additionally, it only works if your validator supports the Standard JSON Schema specification (e.g. via `~standard.jsonSchema` or a custom fallback like `.toJSONSchema()`). Supported validators include:
...
- Any validator implementing the [Standard JSON Schema](https://standardschema.dev/json-schema#what-schema-libraries-support-this-spec) spec.
  :::
