...
:::note
In `arkenv/standard` (the ArkType-free entry point), coercion is supported as an opt-in feature. You must set `coerce: true` in your configuration, and your validator must support Standard JSON Schema (e.g. via `~standard.jsonSchema`).
:::
