...
:::important[Using NestJS?]
Do not validate env with `ConfigModule`. Follow [Use with NestJS](/docs/arkenv/how-to/use-with-nestjs): a fail-fast `env` object, imported at the top of `main.ts`, started with `--env-file`.
:::
...
```typescript title="src/env.ts" twoslash
