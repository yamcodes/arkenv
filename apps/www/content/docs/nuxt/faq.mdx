...
ArkEnv's Nuxt module scans your schema file(s) at build time, identifies the client-side and shared keys, and automatically generates a tailored `createEnv` factory inside `generated/env.gen.ts` that includes the complete, compiled `runtimeEnv` block using Nuxt's `runtimeConfig`. You get the benefits of typesafe validation without manual mapping boilerplate.
