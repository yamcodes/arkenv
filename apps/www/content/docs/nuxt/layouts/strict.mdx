...
When `@arkenv/nuxt/module` runs in strict layout, the server entry auto-merges client and shared variables. You do not need to import `./client` or pass `extends: [clientEnv]` unless you want an explicit override.

The client entry still extends `SharedSchema` explicitly because automatic merging currently applies only to the server entry. The resulting chain is `SharedSchema` → client env → server env, so the server receives shared variables transitively through the client env.

Auto-merge only applies when the `extends` key is **omitted**. Passing `extends` at all - including `extends: []` or a list that does not include `clientEnv` - opts out and uses your list as-is.
...
<Accordions>
  <Accordion title="Manual setup">
    ...
    Add `@arkenv/nuxt/module` to your modules array in `nuxt.config.ts`. This statically scans your schema directory, registers environment variables to Nuxt's `runtimeConfig`, and enables server auto-extend. No generated files are written to disk:
