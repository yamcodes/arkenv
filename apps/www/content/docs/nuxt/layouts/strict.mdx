...
When `@arkenv/nuxt/module` runs in strict layout, omitting `extends` auto-merges:

- **Client entry** includes `SharedSchema` from `env/internal/shared.ts`
- **Server entry** includes the composed client env (and therefore shared variables)

You do not need to import `SharedSchema` or `./client`, or pass `extends`, unless you want an explicit override.

Auto-merge only applies when the `extends` key is **omitted**. Passing `extends` at all - including `extends: []` or a custom list - opts out and uses your list as-is. Explicit lists are not silently augmented with `SharedSchema` or `clientEnv`.
...
<Accordions>
  <Accordion title="Manual setup">
    ...
    Create `env/client.ts` importing from `@arkenv/nuxt/client`. All local keys **must** be prefixed with `NUXT_PUBLIC_`. With the module registered, omitting `extends` auto-merges `SharedSchema`:
    ...
    Any explicit `extends` is an escape hatch that disables auto-merge. Use it to pass a custom list (for example `extends: [clientEnv]`), or `extends: []` if you intentionally want no client merge:
    ...
    Add `@arkenv/nuxt/module` to your modules array in `nuxt.config.ts`. This statically scans your schema directory, registers environment variables to Nuxt's `runtimeConfig`, and enables client/server auto-extend. No generated files are written to disk:
