...
1. **Integration with `runtimeConfig`.** ArkEnv registers client-side and shared variables in `runtimeConfig.public` (making them accessible everywhere) and server-only variables in the private `runtimeConfig` (ensuring they remain strictly on the server).
...
Depending on the layout layout you choose, here is what is actually exposed in the client-side bundle:
