...
Invalid input raises `ArkEnvValidationError`. The message lists every failing path:
...
Most apps let that throw stop the process. You do not need a `try/catch` around every `arkenv()` call. Catch only when you want to inspect `ArkEnvValidationError` yourself (custom boot UI, tests, or a wrapper):
...
Reading a server-only key from client code is a misuse, not a schema failure. The throw is a native `Error` (so Vite/Bun client modules never import `ArkEnvValidationError`). `name` stays `"Error"`. The message is an instruction, same shape as Next.js taint copy, with ArkEnv at the end so agents can attribute it:

```txt
Error: Do not access server-only key 'DATABASE_URL' on the client since it will leak sensitive data (prevented by ArkEnv)
```

Do not catch this. Fix the access. `instanceof ArkEnvValidationError` is for schema failures only — it is `false` here because there are no `issues`.
