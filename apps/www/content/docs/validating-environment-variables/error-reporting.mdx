...
Reading a server-only key from client code raises a **boundary access error**: a native `Error` with `name === "ArkEnvError"`, not an `ArkEnvError` instance (client graphs must not import the class). Stacks still print the same brand as validation failures:

```txt
ArkEnvError: Attempted to access server environment variable 'DATABASE_URL' on the client.
```

Catch validation failures and boundary access together:

```ts
if (error instanceof Error && error.name === "ArkEnvError") {
  // validation failures and boundary access
}
```

`instanceof ArkEnvError` stays validation-only (and gives you `issues`).
