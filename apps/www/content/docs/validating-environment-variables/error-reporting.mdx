...
Reading a server-only key from client code is a misuse, not a schema failure. The throw is a native `Error` (so Vite/Bun client modules never import the `ArkEnvError` class) with `name = "ArkEnvError"` so the stack matches validation:

```txt
ArkEnvError: Attempted to access server environment variable 'DATABASE_URL' on the client.
```

Do not catch this. Fix the access. `instanceof ArkEnvError` is for validation only — it is `false` here because there are no `issues`.
