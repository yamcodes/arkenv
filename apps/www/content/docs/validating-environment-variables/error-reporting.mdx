...
Reading a server-only key from client code is a misuse, not a schema failure. The throw is a native `Error` (so Vite/Bun client modules never import `ArkEnvValidationError`). `name` stays `"Error"`; the message attributes ArkEnv:
