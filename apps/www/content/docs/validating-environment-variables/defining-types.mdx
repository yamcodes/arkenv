...
<Tabs items={["ArkType (@arkenv/core)", "Zod (@arkenv/standard)"]}>
  ...
  <Tab value="Zod (@arkenv/standard)">
    ...
    Valibot (and other Standard Schema libraries) follow the same pattern: pass each field's schema as the map value. For Valibot coercion, see the [Valibot](/docs/guides/validators/valibot) guide. Also [`@arkenv/standard`](/docs/reference/standard).
  </Tab>
</Tabs>
...
See [Transforms](/docs/core-concepts/transforms) for morph vs Zod `.transform`, and [reusing schemas](/docs/validating-environment-variables/reusing-schemas) when the whole schema should be shared.
