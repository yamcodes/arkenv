...
The same `coerce` option works with `@arkenv/standard` when the validator exposes JSON Schema ArkEnv can introspect (Zod 4.2+, Valibot with `@valibot/to-json-schema`, Zod Mini, VineJS 4.3+, and other libraries on the [Standard JSON Schema](https://standardschema.dev/json-schema#what-schema-libraries-support-this-spec) list).
