...
The same `coerce` option works with `@arkenv/standard` when each field is [Standard JSON Schema v1](https://standardschema.dev/json-schema) — not merely Standard Schema. Those specs are orthogonal. Zod 4.2+ and VineJS 4.3+ implement the converter on the schema; Valibot and Zod Mini need wrappers (`toStandardJsonSchema` / `z.toJSONSchema`) so the value you pass is Standard JSON Schema. See the [compatible libraries list](https://standardschema.dev/json-schema#what-schema-libraries-support-this-spec).
