...
The same `coerce` option works with `@arkenv/standard` when each field is [Standard JSON Schema v1](https://standardschema.dev/json-schema) — not merely Standard Schema. Those specs are orthogonal. Zod 4.2+, VineJS 4.3+, Valibot with `@valibot/to-json-schema`, Zod Mini, and other libraries on the [compatible list](https://standardschema.dev/json-schema#what-schema-libraries-support-this-spec) qualify.
