...
1. **Build-time exclusion.** The `withArkEnv` plugin guarantees that only client and shared values are mapped for the browser. Server secrets are deliberately excluded, meaning they are never even shipped to the client-side bundle.
...
- **Values are never leaked.** Next.js handles raw `process.env.X` references; ArkEnv's boundary handles the `env.X` object.
