...
ArkEnv build integrations accept an optional logger and log level. By default, messages use the `[ArkEnv]` prefix and respect the `ARKENV_LOG_LEVEL` environment variable (`debug`, `info`, `warn`, `error`, or `silent`).
