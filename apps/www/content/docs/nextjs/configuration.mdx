...
Or set it up manually by skipping `withArkEnv` in `next.config.ts` and importing `createEnv` directly from `@arkenv/nextjs` in your schema file. You then become responsible for keeping `runtimeEnv` in sync with your schema.
