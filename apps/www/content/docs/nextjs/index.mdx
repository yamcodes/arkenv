...
- **`false` (Client/SSR)** — only the `client + shared` schema is validated. Accessing a key from the `server` schema throws a descriptive error during SSR pre-rendering _and_ in the browser.
