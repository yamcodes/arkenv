---
title: Introduction
icon: Album
description: Validate and secure your Next.js environment variables with zero-boilerplate scaffolding and a seamless developer experience.
---

:::important
This integration requires [ArkType](https://arktype.io) to be installed.
:::

The `@arkenv/nextjs` integration provides end-to-end environment validation for both the server and the browser. It catches configuration bugs early in your build pipeline and prevents sensitive server-side variables from ever reaching client components.

## Quickstart

The fastest way to get started is with the [ArkEnv CLI](/docs/cli). It automatically configures `@arkenv/nextjs` for your existing Next.js project.

<Tabs items={['npm', 'pnpm', 'yarn', 'bun']}>
  <Tab value="npm">
    ```bash
    npx @arkenv/cli@latest init
    ```
  </Tab>

  <Tab value="pnpm">
    ```bash
    pnpm dlx @arkenv/cli@latest init
    ```
  </Tab>

  <Tab value="yarn">
    ```bash
    yarn dlx @arkenv/cli@latest init
    ```
  </Tab>

  <Tab value="bun">
    ```bash
    bunx @arkenv/cli@latest init
    ```
  </Tab>
</Tabs>

## Layout strategies

ArkEnv supports two layout structures to match your security and developer experience preferences:

<Cards>
  <Card 
    title="Simple layout (recommended)" 
    href="/docs/nextjs/layouts/simple"
    description="Define client and server schemas in a single file (src/env.ts) to prioritize developer experience (DX)."
  />
  <Card 
    title="Strict layout" 
    href="/docs/nextjs/layouts/strict"
    description="Separate client and server schemas into different files to prioritize strict compile-time security."
  />
</Cards>

## Next steps

<Cards>
  <Card 
    title="Zod, Valibot, and other Standard Schema validators" 
    href="/docs/nextjs/using-other-validators" 
    description="Learn how to mix and match ArkType with other Standard Schema validators in Next.js." 
  />
  <Card 
    title="FAQ" 
    href="/docs/nextjs/faq" 
    description="Find answers to frequently asked questions about @arkenv/nextjs, including the danger of custom shared variables." 
  />
  <Card 
    title="Next.js Environment Variables Guide" 
    href="https://nextjs.org/docs/pages/guides/environment-variables" 
    description="Read the official Next.js documentation on how environment variables are loaded and exposed." 
  />
</Cards>
