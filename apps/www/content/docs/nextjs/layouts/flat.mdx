...
The values themselves are *not* shipped to the client! But if exposing your server variable *names*, *types*, or *constraints* is a security concern, use the [strict layout](/docs/nextjs/layouts/strict) instead.
:::
...
<Accordions>
  <Accordion title="Manual Setup">
    ...
    :::note
    If you scaffolded your project using the `--no-codegen` CLI option, your `next.config.ts` will already be wrapped with `withArkEnv(nextConfig, { codegen: false })`. This skips `env.gen.ts` generation while keeping build-time validation active, and you become responsible for manually destructuring `runtimeEnv` in `env.ts`.
    :::
