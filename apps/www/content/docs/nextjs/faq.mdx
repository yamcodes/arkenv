...
> If you disable codegen, you will not be using `withArkEnv`. To trigger build-time environment variable validation anyway, simply import your `env.ts` file directly in your `next.config.ts` file:
>
> ```ts title="next.config.ts"
