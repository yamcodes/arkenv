...
Yes. If you prefer to maintain the `runtimeEnv` mapping manually (or if your build environment restricts writing files during builds), you can completely opt out of the code generation workflow.

To do this, remove the `withArkEnv` wrapper from your Next.js configuration. Because `withArkEnv` requires code generation to build the client-side proxy mapping, it cannot be used without generating files.

To trigger build-time environment variable validation anyway, simply import your `env.ts` file directly at the top of your `next.config.ts` file:

```ts title="next.config.ts"
import "./src/env"; // Triggers validation on Next.js build / dev startup
