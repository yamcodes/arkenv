...
Load your `.env` file before executing Node.js so that `process.env` is populated before `src/env.ts` evaluates. With Node.js 20.6+, use the native `--env-file` flag with the Nest CLI:
...
Although NestJS's `validationSchema` option accepts Standard Schema validators, `@nestjs/config` does not perform automatic string coercion. Without coercion, values remain raw strings unless you explicitly define coercion in your schema (e.g., `z.coerce` in Zod). Passing a standard ArkEnv schema will leave values like `PORT` as the string `"3000"` instead of converting them to numbers.
