...
Vite and Bun evaluate two module graphs in full-stack and SSR apps.
...
`@arkenv/standard`) as a peer of the host adapter so you do not ship a
second runtime and can pair any adapter with ArkType or a Standard
Schema validator.
...
Node's `node --env-file=.env` also works. You name each file yourself,
and it does not run TypeScript.
