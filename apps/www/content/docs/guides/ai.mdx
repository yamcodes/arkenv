...
Point the assistant at this page and [Framework integration](/docs/validating-your-environment/framework-integration) when it needs host context.
...
- **Init on an existing repo**: if `.env.example` exists, init can detect
  keys and offer them for the schema (see
  [structuring your schema](/docs/validating-your-environment/structuring-your-schema)).
  Init never reads `.env`.
- **Missing files**: init may create `.env` and `.env.example` with
  framework defaults, or copy `.env.example` to `.env` when only the
  template exists. A missing `.env.example` is filled from detected keys
  and defaults, not from `.env`.
- **After schema changes**: add new keys to `.env.example` with empty or
  placeholder values. Commit the template; keep `.env` and `.env.local`
  gitignored.
...
| Rule                                                                 | Why                                                        |
| -------------------------------------------------------------------- | ---------------------------------------------------------- |
| Run the CLI with `--agent` for setup                                 | Avoids drift from hand-scaffolded config                   |
...
For layout and client/server tradeoffs, link assistants to [Client vs. server](/docs/validating-your-environment/client-vs-server) instead of inventing a third layout.
...
- Load `.env` via the framework or Nub; ArkEnv only validates
