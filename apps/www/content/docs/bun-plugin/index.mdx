...
<Accordions>
  <Accordion title="Manual Installation">
    ...
    :::note
    `@arkenv/bun-plugin` lists `@arkenv/core` as a **peer dependency**. This ensures that the engine resolves to a single root-level instance across your codebase, preventing "dual package hazards" that can break runtime type evaluations and singleton context.
    :::
...
2. Register the plugin programmatically in your build script using `@arkenv/bun-plugin/standard`:
...
3. To use the static dev server plugin with auto-discovery, update `bunfig.toml` to reference `@arkenv/bun-plugin/standard`:
...
The Bun plugin accepts an optional third argument for build-time logging. Use `logLevel: "silent"` to suppress warnings, or provide a custom `logger` with `error`, `warn`, `info`, and `debug` methods:

```ts title="build.ts"
plugins: [arkenv(Env, undefined, { logLevel: "warn" })],
```
