...
:::note
When using external validators like Zod or Valibot, you must install them separately. For example, to use the `z` symbol from `"zod"` shown below, install it via:

```npm
npm install zod
```

(or use `valibot` instead).
:::
