...
/**
 * Drop module-resolution noise and trim ArkType hover text.
 *
 * @param node Twoslash node before line positions are attached
 * @returns Whether the node should stay in the popup output
 */
export function filterTwoslashNode(node: TwoslashFilterNode): boolean {
	switch (node.type) {
		case "hover": {
		...
		case "error": {
			...
			for (const transformation of arkTypePackageJson.contributes
				...
				if (matchResult) {
					let text = transformation.message;
