...
vi.mock("~/lib/roadmap/config", () => ({
	...
	ROADMAP_EXCLUDE_ISSUE_NUMBERS: new Set([683]),
	ROADMAP_EXTRAS: [
		{ id: "release-v1", title: "Release v1", done: false },
...
describe("fetchRoadmap", () => {
	...
	it("merges milestone issues with extras and computes progress", async () => {
		...
		expect(roadmap.totalCount).toBe(4); // 2 issues + 2 extras (683 excluded, PR skipped)
		expect(roadmap.doneCount).toBe(2);
		expect(roadmap.percent).toBe(50);
	...
	it("falls back to extras when GitHub fails", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => {
